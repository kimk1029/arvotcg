...
- GET/HEAD 공개 시세·카탈로그: 스니덩크 시세/검색/랭킹, 팩, 카드 카탈로그, KREAM·한국 시세,
  시장지수, 환율, PSA pop, 카드명 번역, 배너, 장소/샵, 인기 검색어. 회원 데이터가 없다.
- POST 행동/방문/광고 기록, 카드명 번역 변환, 검색어 기록. 익명 ID·출처는 통계용이며 인증 근거로 사용하지 않는다.
...
- 익명 사용자가 받던 회원 콘텐츠·개인 API 조회는 401로 바뀐다. 공개 시세·카탈로그 조회는
  로그인 게이트 이전 스토어 빌드가 미로그인으로 사용하므로 계속 무인증으로 열어둔다
  (2026-09-09 운영에서 앱 조회 401 다발로 확인, `shared/accessPolicy.ts`).
- 신규 앱은 웹뷰 토큰 전달이 헤더 방식이며 구버전 앱 URL 전달도 호환한다.
