    ...
    <Modal visible={!!shop} animationType="slide" onRequestClose={onClose} statusBarTranslucent>
      {shop && (
        <View style={{ flex: 1, backgroundColor: '#fff', paddingTop: insets.top }}>
          ...
          <ScrollView style={{ flex: 1 }} contentContainerStyle={{ paddingBottom: 24 + insets.bottom }} showsVerticalScrollIndicator={false}>
            ...
            <View style={{ paddingHorizontal: 20, paddingTop: 16 }}>
              ...
              <Text style={[t(12.5, '500', '#8E8E93'), { marginTop: 5 }]}>{shop.addr}</Text>
              {/* actions */}
              <View style={{ flexDirection: 'row', gap: 8, marginTop: 14 }}>
                ...
                <ActionBtn bg="#F2F2F4" fg={INK} label="네이버지도" onPress={() => Linking.openURL(`https://map.naver.com/p/search/${encodeURIComponent(shop.name)}`).catch(() => {})}
                  icon={<View style={{ width: 18, height: 18, borderRadius: 5, backgroundColor: '#03C75A', alignItems: 'center', justifyContent: 'center' }}><Text style={t(10, '900', '#fff')}>N</Text></View>} />
                <ActionBtn bg="#F2F2F4" fg={INK} label="티맵" onPress={() => Linking.openURL('https://tmap.life').catch(() => {})}
                  icon={<View style={{ width: 18, height: 18, borderRadius: 5, backgroundColor: '#E8412C', alignItems: 'center', justifyContent: 'center' }}><Text style={t(9, '900', '#fff')}>T</Text></View>} />
              </View>
            </View>
            {/* intro */}
              ...
              <View style={{ backgroundColor: '#F7F7F9', borderRadius: 14, paddingVertical: 14, paddingHorizontal: 16, gap: 9 }}>
                {([
                  ...
                  <View key={k} style={{ flexDirection: 'row', justifyContent: 'space-between', gap: 12 }}>
                    ...
                    <Text style={[t(12.5, '700', INK), { flexShrink: 1, textAlign: 'right' }]}>{v}</Text>
                  </View>
                ))}
              </View>
            </View>
