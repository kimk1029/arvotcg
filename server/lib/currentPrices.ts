...
const writes = new DailyCache<boolean>(DAY_MS, 25000);
const money = (n?: number) => Number.isFinite(n) ? Math.max(0, Math.round(n!)) : 0;

/** Atomic cross-process throttle: only a changed current row appends history. */
export async function saveCurrentPrice(apparelId: number, p: PriceInput): Promise<void> {
  const full = p.priceSingle !== undefined || p.pricePsa10 !== undefined;
  await writes.get(`${apparelId}:${full ? 'full' : 'partial'}`, () => priceWork.run(async () => {
    ...
    `;
    if (changed > 0) for (const notify of listeners) notify(apparelId);
    return true;
  }));
}
...
const currentCache = new DailyCache<CurrentPrice[]>(DAY_MS, 256);
const reads = new WorkQueue(1, 100);
onCurrentPriceChanged(id => currentCache.invalidate(key => key.split(',').includes(String(id))));
export async function readCurrentPrices(ids: number[]): Promise<CurrentPrice[]> {
