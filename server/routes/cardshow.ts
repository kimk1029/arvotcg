...
import { DEFAULT_EVENT_KEY, isEventKey, type EventKey } from '../../shared/eventPages';

/**
 * 카드쇼/트레이드 데이 예약 이벤트 API — 웹 /event/cardshow · /event/tradeday (브라우저 쿠키
 * 또는 앱 웹뷰의 Bearer 토큰)에서 사용. 슬롯/정원은 어드민(admin.arvotcg.com)에서 관리.
 * 행사 구분은 eventKey(shared/eventPages.ts) — 쿼리/바디 `event`, 기본 cardshow.
 * 정책: 유저당 행사마다 예약 1건 — 같은 행사의 다른 슬롯을 예약하면 기존 예약이 이동한다.
 */
const router = Router();

/** 쿼리(?event=) 또는 바디({event}) 의 행사 키. 모르는 값이면 기본(cardshow). */
function eventKeyOf(req: Request): EventKey {
  const q = req.query.event;
  const b = (req.body as { event?: unknown } | null)?.event;
  const v = typeof q === 'string' ? q : b;
  return isEventKey(v) ? v : DEFAULT_EVENT_KEY;
}
  ...
  try {
    ...
    const [slots, events] = await Promise.all([
      prisma.cardShowSlot.findMany({
        where: { active: true, eventKey },
        orderBy: [{ date: 'asc' }, { time: 'asc' }],
        include: { _count: { select: { reservations: true } } },
      }),
      // 날짜별 행사 정보(행사명·장소·시간 등) — 어드민에서 관리. 없는 날짜는 웹이 기본값.
      prisma.cardShowEvent.findMany({ where: { eventKey }, orderBy: { date: 'asc' } }),
    ]);
...
router.post('/reserve', requireAuth, async (req: Request, res: Response) => {
  ...
  try {
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      // 같은 행사(eventKey)의 기존 예약만 이동 대상 — 다른 행사 예약은 그대로 둔다.
      const eventKey = slot.eventKey;
      const existing = await tx.cardShowReservation.findUnique({ where: { userId_eventKey: { userId, eventKey } } });
...
/** DELETE /api/cardshow/reserve?event= — 해당 행사의 내 예약 취소. */
router.delete('/reserve', requireAuth, async (req: Request, res: Response) => {
  try {
    const eventKey = eventKeyOf(req);
    const existing = await prisma.cardShowReservation.findUnique({
      where: { userId_eventKey: { userId: req.user!.userId, eventKey } },
    });
    if (existing?.checkedInAt) {
      return res.status(409).json({ error: '이미 입장 완료된 예약은 취소할 수 없어요' });
    }
    await prisma.cardShowReservation.deleteMany({ where: { userId: req.user!.userId, eventKey } });
    res.json({ ok: true });
  } catch (err) {
...
/** POST /api/cardshow/check-in { event } — 현장에서 내 예약 화면을 담당자가 확인한 뒤 입장 완료 처리. */
