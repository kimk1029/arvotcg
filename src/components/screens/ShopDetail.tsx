...
export function ShopDetail({ shop, onClose }: Props) {
  ...
  const share = () => {
    ...
    if (typeof navigator !== 'undefined' && navigator.share) navigator.share({ title: shop.name, text, url }).catch(() => {});
    else if (typeof navigator !== 'undefined' && navigator.clipboard) navigator.clipboard.writeText(`${text}\n${url}`).catch(() => {});
  };
  const actionBtn = (bg: string, fg: string, label: string, icon: React.ReactNode, onClick?: () => void, href?: string) => {
    const st: React.CSSProperties = { flex: 1, height: 42, borderRadius: 12, background: bg, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, cursor: 'pointer', border: 'none', textDecoration: 'none', fontSize: 13, fontWeight: 800, color: fg };
    return href
      ? <a href={href} target={href.startsWith('tel:') ? undefined : '_blank'} rel="noreferrer" style={st}>{icon}{label}</a>
      : <button type="button" onClick={onClick} style={st}>{icon}{label}</button>;
  };
    ...
    <div style={{ position: 'fixed', inset: 0, zIndex: 60, background: '#fff', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
      ...
      <div className="cv-hrow" style={{ overflowY: 'auto', flex: 1, minHeight: 0 }}>
        ...
        <div style={{ padding: '16px 20px 0' }}>
          ...
          <div style={{ fontSize: 12.5, color: '#8E8E93', fontWeight: 500, marginTop: 5 }}>{shop.addr}</div>
          {/* actions */}
          <div style={{ display: 'flex', gap: 8, marginTop: 14 }}>
            {shop.phone && actionBtn(INK, '#fff', '전화', <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="#fff" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round"><path d="M22 16.9v3a2 2 0 0 1-2.2 2 19.8 19.8 0 0 1-8.6-3.1 19.5 19.5 0 0 1-6-6A19.8 19.8 0 0 1 2.1 4.2 2 2 0 0 1 4.1 2h3a2 2 0 0 1 2 1.7c.1.9.4 1.8.7 2.7a2 2 0 0 1-.5 2.1L8.1 9.7a16 16 0 0 0 6 6l1.3-1.3a2 2 0 0 1 2.1-.4c.9.3 1.8.6 2.7.7a2 2 0 0 1 1.7 2z" /></svg>, undefined, `tel:${shop.phone.replace(/[^\d+]/g, '')}`)}
            {actionBtn('#F2F2F4', INK, '네이버지도', <span style={{ width: 18, height: 18, borderRadius: 5, background: '#03C75A', color: '#fff', fontSize: 10, fontWeight: 900, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>N</span>, undefined, `https://map.naver.com/p/search/${encodeURIComponent(shop.name)}`)}
            {actionBtn('#F2F2F4', INK, '티맵', <span style={{ width: 18, height: 18, borderRadius: 5, background: '#E8412C', color: '#fff', fontSize: 9, fontWeight: 900, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>T</span>, undefined, 'https://tmap.life')}
          </div>
        </div>
        {/* intro */}
